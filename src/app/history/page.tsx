    ...
    <div className="relative min-h-screen overflow-hidden bg-[#f8fafc]">
      ...
      <main className="relative z-10 container mx-auto px-4 py-24 md:py-32">
        <div className="mx-auto max-w-7xl">
          {/* Header */}
          <div className="mb-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 animate-fade-in-up">
            <div>
            ...
            <div className="flex gap-3">
              {history.length > 0 && (
                <button
                  ...
                  className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white/50 backdrop-blur-sm px-4 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-white hover:text-rose-600 hover:border-rose-200 shadow-sm"
                >
              ...
              <Link
                href="/analyzer"
                className="flex items-center gap-2 rounded-xl bg-emerald-600 px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-emerald-200/50 transition hover:bg-emerald-700 hover:scale-105 hover:shadow-emerald-300/50"
              >
          ...
          {history.length === 0 ? (
            <div className="glass-panel rounded-3xl p-16 text-center animate-fade-in-up delay-100">
              <div className="mx-auto mb-6 flex h-24 w-24 items-center justify-center rounded-full bg-emerald-50 border border-emerald-100">
                <History className="h-10 w-10 text-emerald-600/50" />
              </div>
              <h3 className="mb-3 text-2xl font-bold text-slate-900">No History Yet</h3>
              ...
              <Link
                href="/analyzer"
                className="inline-flex items-center gap-2 rounded-full bg-emerald-600 px-8 py-4 font-semibold text-white shadow-xl shadow-emerald-200/50 transition hover:bg-emerald-700 hover:scale-105"
              >
            ...
            <div className="grid gap-8 lg:grid-cols-[1fr_1.5fr] animate-fade-in-up delay-100">
              {/* History List */}
              <div className="glass-panel rounded-3xl p-6 h-[calc(100vh-200px)] flex flex-col">
                  ...
                  {history.map((entry) => {
                    ...
                    return (
                      <div key={entry.id} className="group relative">
                        ...
                        >
                          <div className={`rounded-2xl border p-5 transition-all duration-200 ${isSelected
                              ? 'border-emerald-500 bg-emerald-50/50 shadow-md'
                              : 'border-slate-100 bg-white/50 hover:border-emerald-200 hover:bg-white hover:shadow-sm'
                            }`}>
                            <div className="mb-3 flex items-start justify-between">
                              <div className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-bold uppercase tracking-wide ${chip.bgClass} ${chip.className}`}>
              ...
              {/* Detail View */}
              <div className="glass-panel rounded-3xl p-8 h-[calc(100vh-200px)] overflow-y-auto custom-scrollbar">
                {selectedEntry ? (
                  <div className="space-y-8">
                    <div className="flex items-start justify-between border-b border-slate-100 pb-6">
                      <div>
                        <div className="flex items-center gap-3 mb-2">
                          <h2 className="text-3xl font-bold text-slate-900">Analysis Details</h2>
                      ...
                      <div className={`flex flex-col items-center justify-center h-20 w-20 rounded-2xl ${selectedEntry.analysis.healthScore >= 70 ? 'bg-emerald-100 text-emerald-700' : selectedEntry.analysis.healthScore >= 40 ? 'bg-amber-100 text-amber-700' : 'bg-rose-100 text-rose-700'}`}>
                        <span className="text-3xl font-bold">{selectedEntry.analysis.healthScore}</span>
                        <span className="text-[10px] font-bold uppercase tracking-wide opacity-80">Score</span>
                      </div>
                    </div>

                    <div className="bg-emerald-50/50 rounded-2xl p-6 border border-emerald-100">
                      <h3 className="mb-3 font-bold text-emerald-800 flex items-center gap-2">
                    ...
                    <div>
                      <h3 className="mb-3 font-bold text-slate-900 text-lg">Overall Assessment</h3>
                    ...
                    {selectedEntry.analysis.ingredients && (
                      <div>
                        <h3 className="mb-3 font-bold text-slate-900 text-lg">Detected Ingredients</h3>
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                      {selectedEntry.analysis.pros.length > 0 && (
                        <div>
                          <h3 className="mb-4 font-bold text-emerald-700 flex items-center gap-2">
                      ...
                      {selectedEntry.analysis.cons.length > 0 && (
                        <div>
                          <h3 className="mb-4 font-bold text-rose-700 flex items-center gap-2">
                    ...
                    {selectedEntry.analysis.recommendations.length > 0 && (
                      <div>
                        <h3 className="mb-4 font-bold text-slate-900 text-lg">Recommendations</h3>
